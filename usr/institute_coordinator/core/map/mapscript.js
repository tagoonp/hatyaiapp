...
function loadingInstitute(){
  $.getJSON( "system/call-institute.php", function( data ) {
      ...
      $.each(obj_marker,function(i,k){
        ...
        infowindow_inst[i] = new google.maps.InfoWindow({
          content:$.ajax({
        ...
        google.maps.event.addListener(my_Marker_inst[i], 'click', function(){ // เมื่อคลิกตัว marker แต่ละตัว
          if(infowindowTmp){ // ให้ตรวจสอบว่ามี infowindow ตัวไหนเปิดอยู่หรือไม่
                          infowindow_inst[infowindowTmp].close();  // ถ้ามีให้ปิด infowindow ที่เปิดอยู่
          }
          infowindow_inst[i].open(map,my_Marker_inst[i]); // แสดง infowindow ของตัว marker ที่คลิก
...
function loadingMarker(){
  $.getJSON( "system/call-red-marker.php", function( data ) {
      ...
      $.each(obj_marker,function(i,k){
        ...
        //  กรณีนำไปประยุกต์ ดึงข้อมูลจากฐานข้อมูลมาแสดง
        infowindow_r[i] = new google.maps.InfoWindow({
          content:$.ajax({
              url:'system/placeDetail.php',//ใช้ ajax ใน jQuery ดึงข้อมูล
              data:'placeID='+obj_marker[i].alt_id,// ส่งค่าตัวแปร ไปดึงข้อมูลจากฐานข้อมูล
              async:false
          }).responseText
        });

        google.maps.event.addListener(my_Marker_r[i], 'click', function(){ // เมื่อคลิกตัว marker แต่ละตัว
          if(infowindowTmp){ // ให้ตรวจสอบว่ามี infowindow ตัวไหนเปิดอยู่หรือไม่
                          infowindow_r[infowindowTmp].close();  // ถ้ามีให้ปิด infowindow ที่เปิดอยู่
          }
          infowindow_r[i].open(map,my_Marker_r[i]); // แสดง infowindow ของตัว marker ที่คลิก
          infowindowTmp=i; // เก็บ infowindow ที่เปิดไว้อ้างอิงใช้งาน
        });
  ...
  $.getJSON( "system/call-yellow-marker.php", function( data ) {
      ...
      $.each(obj_marker,function(i,k){
        ...
        infowindow_y[i] = new google.maps.InfoWindow({
          content:$.ajax({
              url:'system/placeDetail.php',//ใช้ ajax ใน jQuery ดึงข้อมูล
              data:'placeID='+obj_marker[i].alt_id,// ส่งค่าตัวแปร ไปดึงข้อมูลจากฐานข้อมูล
              async:false
        ...
        google.maps.event.addListener(my_Marker_y[i], 'click', function(){ // เมื่อคลิกตัว marker แต่ละตัว
          if(infowindowTmp){ // ให้ตรวจสอบว่ามี infowindow ตัวไหนเปิดอยู่หรือไม่
                          infowindow_y[infowindowTmp].close();  // ถ้ามีให้ปิด infowindow ที่เปิดอยู่
          }
          infowindow_y[i].open(map,my_Marker_y[i]); // แสดง infowindow ของตัว marker ที่คลิก
          infowindowTmp=i; // เก็บ infowindow ที่เปิดไว้อ้างอิงใช้งาน
        });
  ...
  $.getJSON( "system/call-green-marker.php", function( data ) {
      ...
      $.each(obj_marker,function(i,k){
        ...
        //  กรณีตัวอย่าง ดึง title ของตัว marker มาแสดง
        infowindow_g[i] = new google.maps.InfoWindow({
          content:$.ajax({
              url:'system/placeDetail2.php',//ใช้ ajax ใน jQuery ดึงข้อมูล
              data:'placeID='+obj_marker[i].alt_id,// ส่งค่าตัวแปร ไปดึงข้อมูลจากฐานข้อมูล
              async:false
          }).responseText
        });

        google.maps.event.addListener(my_Marker_g[i], 'click', function(){ // เมื่อคลิกตัว marker แต่ละตัว
          if(infowindowTmp){ // ให้ตรวจสอบว่ามี infowindow ตัวไหนเปิดอยู่หรือไม่
                          infowindow_g[infowindowTmp].close();  // ถ้ามีให้ปิด infowindow ที่เปิดอยู่
          }
          infowindow_g[i].open(map,my_Marker_g[i]); // แสดง infowindow ของตัว marker ที่คลิก
          infowindowTmp=i; // เก็บ infowindow ที่เปิดไว้อ้างอิงใช้งาน
        });
